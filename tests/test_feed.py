...
def test_only_an_occurrence_suffix_marks_a_name_stale():
    assert is_stale("Kestrin.0 Vantel") and is_stale("Zenobe.12 Vantel")
    assert not is_stale("Harrowmere Odalys. Harrowmere")
    assert not is_stale("Tariq Zorvane")
    assert person_url("example", "Harrowmere Odalys. Harrowmere").endswith(
        "n=harrowmere&p=harrowmere+odalys.&oc=0"
    )
